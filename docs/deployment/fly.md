...
**Status:** executed. VNI is live at <https://vni.fly.dev> on app `vni` and
unmanaged Postgres Flex `vni-db`, both in the `personal` org, region `iad`.
Phases 1–5 are complete; Phase 6 (custom domain) is not started — no domain
has been chosen yet.

**Last verified:** 2026-07-19, against the deployed stack. Superseded values
from the planning pass are corrected in place below.

## What actually shipped

| Decision | Value |
|---|---|
| `VNI_FLY_ORG` | `personal` |
| `VNI_FLY_APP` | `vni` |
| `VNI_FLY_DB` | `vni-db` |
| `VNI_HOST` | `vni.fly.dev` |
| Database image | `flyio/postgres-flex:17.7` (pinned, not floating `:17`) |
| Database engine | PostgreSQL 17.9, PostGIS 3.6.2 |
| App machine | 1 × `shared-cpu-1x`, 512 MB, ~218 MB used at rest |
| DB machine | 1 × `shared-cpu-1x`, 512 MB, ~236 MB used at rest |
| Volume | `vol_r7y6yxo9y1e7o89r`, 10 GB, encrypted |

Deviations from the plan as written, each deliberate:

- **The image tag is pinned to `17.7`,** not `:17`. A floating major tag makes
  the cluster non-deterministic across recreates.
- **The dump was restored as the `vni` role,** not `postgres`. The app role now
  owns its own tables, so dropping superuser before the pledge phase is a
  privilege change rather than a re-ownership migration.
- **`spatial_ref_sys` data is excluded from the dump.** It is PostGIS
  extension-owned, and the target's own 3.6.2 copy is authoritative. VNI stores
  only SRID 4326 and constructs in 5070; both are standard entries.
- **`auto_stop_machines = "off"`,** not `"stop"`. With one machine and
  `min_machines_running = 1`, `"off"` states the intent directly.
- **`ERL_AFLAGS` lives in `rel/env.sh.eex`, not `fly.toml`,** guarded on
  `FLY_PRIVATE_IP` so the same release still boots under plain `docker run`.
- **The recovery drill is deferred,** not skipped. See "Recovery procedure".

### Compile-time dependency the stock Dockerfile misses

`VNIWeb.SourcesLive` reads `docs/data-sources.md` at compile time via
`@external_resource`. The Dockerfile generated by `mix phx.gen.release --docker`
copies only `mix.exs`/`mix.lock`, `config`, `priv`, `lib`, `assets`, and `rel` —
so the build fails on `mix compile` until `COPY docs docs` is added. Anything
else that gains a compile-time file outside those paths needs the same
treatment.

### PostGIS version drift

Local development runs PostGIS 3.5.3; production runs 3.6.2. This promotion
restores **precomputed** scores rather than recomputing them, so production
carries the 3.5.3-computed values, and the `LEAST(1.0, ...)` wrap already
absorbs the cross-version float noise. Running `mix vni.score` against
production could shift values in the last decimal with no methodology change.
Don't — re-score locally and promote a new dump.
...
Baseline as promoted on 2026-07-19, verified identical on both sides of the
restore. The TK-009 multi-congress work moved these well past the planning-pass
numbers (which recorded 95 MB and a single congress):

- source database: 343 MB; dump: 165 MB custom-format;
- 1,305 districts and 1,305 district scores (435 × congresses 117, 118, 119);
- 435 district profiles (current congress only);
- 150 map versions, 50 with `effective_until IS NULL`;
- 1,250 state-cycle result rows, including all 50 states for 2024;
- 13 migrations; 0 Oban jobs;
- methodology 2026.4;
- compactness rank 1 is `fl-27` (composite 0.987420), rank 429 is `hi-2`
  (composite 0.032706);
...
**Drill status (2026-07-19): deferred, deliberately.** A backup that has never
been restored is a rumor with a timestamp — but the launch dump *was* restored,
end to end, into this cluster, and it verified against the manifest exactly.
That is the recovery path, exercised. What has not been drilled is restoring
Fly's own WAL backup or volume snapshot into a fresh cluster.

WAL archiving is confirmed live rather than merely configured: `pg_stat_archiver`
reported 33 segments archived and 0 failures within the first hour, against a
7-day recovery window, 24h full-backup frequency, and minimum redundancy 3.

Run the full drill before VNI accepts its first durable user write. At that
point the dump stops being a sufficient recovery artifact, because it no longer
contains everything that would be lost.
