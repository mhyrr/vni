...
#     E.g.: docker.io/hexpm/elixir:1.19.4-erlang-28.2-debian-trixie-20260610-slim
...
# VNIWeb.SourcesLive parses docs/data-sources.md at compile time
# (@external_resource) so the /sources page can never drift from the ingest
# registry. That puts a compile-time dependency outside the paths the stock
# Phoenix Dockerfile copies, so it has to come across explicitly.
COPY docs docs
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/vni ./
